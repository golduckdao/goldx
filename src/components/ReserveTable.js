...
export default function ReserveTable() {
  const { buyTokenContractAddress, isAuthenticated, provider } = useStore();
  const [isLoading, setIsLoading] = React.useState(false);
  const [page, setPage] = React.useState(0);
  const [tablerows, setTablerows] = React.useState([]);
  const headers = ["Locked Date", "Reserved Tokens", "Unlock Date", ""];
    ...
    async function fetchData() {
      if (isAuthenticated) {
        ...
        userTickets.forEach((ticket) =>
          promiseArr.push(buyTokenContract.userLockInfo(ticket))
        );
        ...
        rows = rows.map(
          ({ unLockedTime, lockedAmount, lockedTime }, index) => ({
            ...
            action: (
              <Button
                sx={{ color: "#33AEC1" }}
                onClick={async () =>
                  await buyTokenContract.claim(userTickets[index])
                }
              >
                <b>Claim</b>
              </Button>
        ...
        rows = rows.map(({ unlockedTime, lockedAmount, lockedTime }) => [
          `${lockedTime.getUTCDate().toString()}/${
            lockedTime.getUTCMonth() + 1
          }/${lockedTime.getUTCFullYear()} - ${lockedTime.getUTCHours()}:${lockedTime.getUTCMinutes()}`,
          lockedAmount,
          `${unlockedTime.getUTCDate().toString()}/${
            unlockedTime.getUTCMonth() + 1
          }/${unlockedTime.getUTCFullYear()} - ${unlockedTime.getUTCHours()}:${unlockedTime.getUTCMinutes()}`,
        ]);
    ...
    <>
      ...
      >
        <Table sx={{ minWidth: 400 }} aria-label="simple table">
          ...
          {tablerows && tablerows.length > 0 && !isLoading && (
            <TableBody>
              {tablerows.slice(page * 4, page * 4 + 4).map((row, index) => (
                <TableRow
